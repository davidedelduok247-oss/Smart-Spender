...
// Ensure previous signup steps were completed
if (
    empty($_SESSION['signup_full_name']) ||
    empty($_SESSION['signup_email']) ||
    empty($_SESSION['signup_password']) ||
    empty($_SESSION['signup_Q1']) ||
    empty($_SESSION['signup_Q2']) ||
    empty($_SESSION['signup_Q3']) ||
    empty($_SESSION['signup_Q4'])
) {
...
<body>
  ...
  <main class="signup-main container">
    ...
    <form class="signup-form" action="" method="post">
      <h2>How often do you receive income?</h2>

      <div class="qbtn-group">
        <label class="qbtn">
          <input type="radio" name="income_frequency" value="Monthly" required>
          Monthly
        </label>
        <label class="qbtn">
          <input type="radio" name="income_frequency" value="Biweekly">
          Biweekly
        </label>
        <label class="qbtn">
          <input type="radio" name="income_frequency" value="Weekly">
          Weekly
        </label>
        <label class="qbtn">
          <input type="radio" name="income_frequency" value="Irregular">
          Irregular
        </label>
      </div>

      <h3>What is your annual salary?</h3>
      <input
        id="annual_salary"
        name="annual_salary"
        type="number"
